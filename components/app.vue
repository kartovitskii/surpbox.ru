...
export default {
  created() {
    // ну тип тут мы это, из кук пушим в массив данные с корзины
    var cooka = this.readCookie("cart"); // тут я объявил переменную для кук с корзиной
    if (cooka.length > 0) {
      // проверяем есть ли в куке че нить
      ...
      for (var i = 0; i < json_data.length; i++) {
        // перебираем куку и разбиваем на классы (объекты)
        this.cartarray.push({
          name: json_data[i].name,
          price: json_data[i].price,
          id: json_data[i].id
        }); // пушим в массив с остальными объектами корзины
  ...
  methods: {
     deleteitemincart(index) {
       console.log(index)
           this.cartarray = this.cartarray.filter(cartarray => cartarray.id !== index);
            this.calcsum()
    ...
    readCookie(name) {
      var bOut = false;
      return unescape(
        document.cookie.replace(/([^ ;=]+)=?([^;]+)?;?/g, function(m, n, v) {
          if (n == name && !bOut) {
            bOut = true;
            return v;
          } else {
